...
from database.postservice import add_new_post_db, delete_post_db, edit_post_text_db, \
                                 get_exact_post_db, get_all_posts_db, like_post_db, \
                                 unlike_post_db
...
async def change_post(post_id: int = Body(...),
                      new_text: str = Body(...)):
    result = edit_post_text_db(post_id, new_text)
...
async def new_post(user_id: int = Body(...),
    ...
    result = add_new_post_db(user_id=user_id,
                             post_text=post_text,
