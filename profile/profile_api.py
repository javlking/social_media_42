...
from database.userservice import get_exact_user_db, get_all_users_db, \
                                edit_user_info_db, upload_profile_photo_db, \
                                delete_profile_photo_db


# Запрос на получение информации об определенном пользователи
...
async def edit_user(user_id: int = Body(...),
                    new_info: str = Body(...),
                    edit_info: str = Body(...)):

    result = edit_user_info_db(user_id=user_id,
                               edit_info=edit_info,
                               new_info=new_info)
